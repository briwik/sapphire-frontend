...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInCandy } = lotteryNodeData
    prizeInBusd = amountCollectedInCandy.times(cakePriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" SAP"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInCandy)}
            decimals={0}
          />
        )}
      </>
